...
__device__ double find_u(double *u, int x, int y, int z, const Block &block, double *to_recv, Block *recv, int d_size) {
    ...
    for (int r_i = 0; r_i < d_size; r_i++) {
        ...
        return to_recv[offset + local_index(x, y, z, block2)];
    }
    return 1;
}
...
__device__ double laplace_operator(double *u, int x, int y, int z, const Block &block, const Grid &grid, double *to_recv,
    ...
    double dz = (find_u(u, x, y, z - 1, block, to_recv, recv, d_size) - 2 * u[local_index(x, y, z, block)] + find_u(u, x, y, z + 1, block, to_recv, recv, d_size)) / (grid.H_z * grid.H_z);
    return dx + dy + dz;
}
...
void fill_borders(thrust::device_vector<double> &u_device, const Grid grid, const Block block) {
    ...
    if (block.x_max == grid.N)
        fill_first_kind_borders<<<((block.y_size * block.z_size + threads - 1) / threads), threads>>>(thrust::raw_pointer_cast(&u_device[0]), block, X, grid.N, block.y_min, block.z_min, block.y_size, block.z_size);
    ...
    if (block.y_max == grid.N)
        fill_first_kind_borders<<<((block.x_size * block.z_size + threads - 1) / threads), threads>>>(thrust::raw_pointer_cast(&u_device[0]), block, Y, grid.N, block.x_min, block.z_min, block.x_size, block.z_size);
    ...
    if (block.z_max == grid.N)
        fill_first_kind_borders<<<((block.x_size * block.y_size + threads - 1) / threads), threads>>>(thrust::raw_pointer_cast(&u_device[0]), block, Z, grid.N, block.x_min, block.y_min, block.x_size, block.y_size);
}
...
__global__ void calculate_u0(double *u0, double a_t, int size, int x_min, int y_min, int z_min, int y_size, int z_size, const Grid grid, const Block block) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size)
        return;

    int x = x_min + idx / (y_size * z_size);
    int y = y_min + idx % (y_size * z_size) / z_size;
    int z = z_min + idx % z_size;

    u0[local_index(x, y, z, block)] = phi(x * grid.H_x, y * grid.H_y, z * grid.H_z, a_t, grid);
}

__global__ void calculate_u1(double *u0, double *u1, double *to_recv, Block *recv, int d_size,
                             int size, int x_min, int y_min, int z_min, int y_size, int z_size, const Grid grid, const Block block) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size)
        return;
    ...
    u1[local_index(x, y, z, block)] = u0[local_index(x, y, z, block)] + grid.tau * grid.tau / 2 * laplace_operator(u0, x, y, z, block, grid, to_recv, recv, d_size);
}
...
void fill_blocks(const Grid grid, const Block block, double a_t, thrust::device_vector<double> &u0_device, thrust::device_vector<double> &u1_device, thrust::host_vector<Block> &send, thrust::host_vector<Block> &recv, thrust::host_vector<int> &ranks) {
    ...
    int x_max = std::min(block.x_max, grid.N - 1);
    int x_size = x_max - x_min + 1;
    int y_min = std::max(block.y_min, 1);
    int y_max = std::min(block.y_max, grid.N - 1);
    int y_size = y_max - y_min + 1;
    int z_min = std::max(block.z_min, 1);
    int z_max = std::min(block.z_max, grid.N - 1);
    int z_size = z_max - z_min + 1;
    int size = x_size * y_size * z_size;

    calculate_u0<<<(size + threads - 1) / threads, threads>>>(thrust::raw_pointer_cast(&u0_device[0]), a_t, size, x_min, y_min, z_min, y_size, z_size, grid, block);
    ...
    calculate_u1<<<(size + threads - 1) / threads, threads>>>(thrust::raw_pointer_cast(&u0_device[0]),
                                                              ...
                                                              size, x_min, y_min, z_min, y_size, z_size, grid, block);
}
...
void calculate_u(int step, const Grid grid, const Block block, std::vector< thrust::device_vector<double> > &u, thrust::host_vector<Block> &send, thrust::host_vector<Block> &recv, thrust::host_vector<int> &ranks) {
    ...
    calculate_u_kernal<<<(size + threads - 1) / threads, threads>>>(thrust::raw_pointer_cast(&u[step % 3][0]), thrust::raw_pointer_cast(&u[(step + 1) % 3][0]), thrust::raw_pointer_cast(&u[(step + 2) % 3][0]), thrust::raw_pointer_cast(&to_recv_device[0]),thrust::raw_pointer_cast(&recv_device[0]), recv.size(), size, x_min, y_min, z_min, y_size, z_size, grid, block);
...
__global__ void eval_error_kernel(double *u, double tau, double a_t, const Block block, const Grid grid) {
    int idx = threadIdx.x;
    for (int index = idx; index < block.size; index += threads) {
        int x = block.x_min + index / (block.y_size * block.z_size);
        int y = block.y_min + index % (block.y_size * block.z_size) / block.z_size;
        int z = block.z_min + index % block.z_size;
        u[local_index(x, y, z, block)] = fabs(u[local_index(x, y, z, block)] - u_analytical(x * grid.H_x, y * grid.H_y, z * grid.H_z, tau, a_t, grid));
    }
}

double eval_error(thrust::device_vector<double> &u_device, double tau, const Grid grid, const Block block, double a_t) {
    eval_error_kernel<<<1, threads>>>(thrust::raw_pointer_cast(&u_device[0]), tau, a_t, block, grid);
...
double solve_equation(Grid &grid, int steps) {
    ...
    split_block(0, grid.N, 0, grid.N, 0, grid.N, size, X, blocks);
    Block block = blocks[rank];
